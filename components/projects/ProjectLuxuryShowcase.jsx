...
function buildImageSlides(project) {
  ...
  const validGalleryImages = galleryMedia.filter(
    (url) => typeof url === 'string' && url.trim() !== '' && !url.includes('/video/upload/')
  );
  ...
  return (
    <div className="relative flex min-h-[52vh] flex-col lg:min-h-full">
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-black/15 via-transparent to-black/40 lg:to-black/60" aria-hidden />
        ...
        {slides.length > 1 ? (
          <>
            <button type="button" onClick={prev} aria-label="Previous image" className="group absolute left-3 top-1/2 z-10 -translate-y-1/2 flex h-11 w-11 items-center justify-center rounded-full border border-white/15 bg-black/40 text-white/70 backdrop-blur-sm transition-all hover:border-[#C5A880]/60 hover:bg-black/60 hover:text-white active:scale-[0.94] lg:left-5 lg:h-12 lg:w-12">
              <ChevronLeft className="h-5 w-5" strokeWidth={1.5} />
            </button>
            <button type="button" onClick={next} aria-label="Next image" className="group absolute right-3 top-1/2 z-10 -translate-y-1/2 flex h-11 w-11 items-center justify-center rounded-full border border-white/15 bg-black/40 text-white/70 backdrop-blur-sm transition-all hover:border-[#C5A880]/60 hover:bg-black/60 hover:text-white active:scale-[0.94] lg:right-5 lg:h-12 lg:w-12">
        ...
        {slides.length > 1 ? (
          <div className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1.5 lg:hidden">
            {slides.map((_, i) => (
              <button key={i} type="button" onClick={() => onGo(i)} aria-label={`Go to image ${i + 1}`} className={`h-1.5 rounded-full transition-all duration-300 ${i === activeIndex ? 'w-5 bg-[#C5A880]' : 'w-1.5 bg-white/40 hover:bg-white/70'}`} />
            ))}
          </div>
        ) : null}
      </div>
            ...
            {slides.map((url, index) => (
              <button
                ...
                className={`relative h-14 w-[4.5rem] shrink-0 snap-start overflow-hidden rounded-md border-2 transition-all duration-300 sm:h-16 sm:w-[5.25rem] ${
                  index === activeIndex
                    ? 'border-[#C5A880] shadow-[0_0_12px_rgba(197,168,128,0.35)] scale-[1.02]'
                    : 'border-white/12 opacity-50 hover:border-white/35 hover:opacity-100'
                }`}
...
/**
 * FullscreenLightbox — Cinematic Scale + Fade Entry
 */
function FullscreenLightbox({ slides, initialIndex, onClose }) {
  ...
  const isCurrentCloudinaryAsset = typeof currentSrc === 'string' && currentSrc.includes('res.cloudinary.com');

  // Cinematic Entry Trigger
  useEffect(() => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[2147483647] flex items-center justify-center bg-black/98 backdrop-blur-xl transition-opacity duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${isMounted ? 'opacity-100' : 'opacity-0'}`}
      onClick={onClose}
    >
      {/* Close button */}
...
export default function ProjectLuxuryShowcase({ project, backHref = '/pakistan-projects', priceLine = null }) {
  const slides = useMemo(() => buildImageSlides(project), [project]);
  const [activeIndex, setActiveIndex] = useState(0);
  const [fullscreenIndex, setFullscreenIndex] = useState(null);

  useEffect(() => {
    const rafId = window.requestAnimationFrame(() => setActiveIndex(0));
  ...
  const descriptionExcerpt = project.description && project.description.length > 320 ? `${project.description.slice(0, 317).trim()}…` : project.description || '';

  return (
    <>
      {fullscreenIndex !== null && (
        <FullscreenLightboxPortal>
          <FullscreenLightbox slides={slides} initialIndex={fullscreenIndex} onClose={() => setFullscreenIndex(null)} />
        </FullscreenLightboxPortal>
      )}
      
      <section className="relative bg-black pt-24 lg:pt-28" aria-label={`${project.title} showcase`}>
        <div className="grid min-h-[calc(100dvh-5.5rem)] lg:min-h-0 lg:h-[min(78vh,780px)] lg:grid-cols-[minmax(0,1fr)_minmax(320px,36%)]">
          <CarouselImage slides={slides} activeIndex={activeIndex} onGo={setActiveIndex} onFullscreen={setFullscreenIndex} />
          ...
          <aside className="relative flex flex-col border-t border-white/10 bg-[#050505] px-6 py-9 sm:px-8 sm:py-11 lg:h-full lg:overflow-y-auto lg:border-l lg:border-t-0 lg:px-9 lg:py-10 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            <div className="relative flex items-start justify-between gap-4">
              <p className="font-[family-name:var(--font-playfair)] text-base tracking-[0.2em] text-white sm:text-lg">The Plot Sale</p>
              <Link href={backHref} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/25 text-white/80 transition-colors hover:border-[#C5A880] hover:text-[#C5A880]" aria-label="Close and return to listing">
                <X className="h-4 w-4" strokeWidth={1.25} />
              </Link>
            </div>

            {project.primaryLogo ? (<div className="relative mt-6"><ListingLogo src={project.primaryLogo} name={project.title} className="h-11 w-11 sm:h-12 sm:w-12" /></div>) : null}

            <h1 className="relative mt-5 font-[family-name:var(--font-playfair)] text-[clamp(1.65rem,2.8vw,2.25rem)] font-light italic leading-[1.08] text-white">{project.title}</h1>

            {project.location ? (<p className="relative mt-2.5 font-[family-name:var(--font-manrope)] text-[11px] font-light tracking-[0.18em] text-[#C5A880]/70">{project.location}</p>) : null}

            <p className="relative mt-4 font-[family-name:var(--font-manrope)] text-[13.5px] font-light leading-relaxed text-white/62 sm:text-sm">{descriptionExcerpt || 'Premium development curated by The Plot Sale. Full details below.'}</p>

            {priceLine ? (<p className="relative mt-5 inline-flex rounded-full border border-[#C5A880]/40 bg-[#C5A880]/10 px-4 py-1.5 font-[family-name:var(--font-manrope)] text-[13px] font-semibold tracking-wide text-[#E8DCC4]">{priceLine}</p>) : null}

            <div className="relative mt-6">
              <Link href="/contact" className="lux-button inline-flex w-full items-center justify-center rounded-full bg-[#f5f0e8] py-3 font-[family-name:var(--font-manrope)] text-[10.5px] font-semibold uppercase tracking-[0.22em] text-[#0a0a0a] transition-colors hover:bg-white sm:w-auto sm:min-w-[200px] sm:px-9">
                Book appointment
              </Link>
            </div>
            ...
            {areaStat ? (
              <div className="relative mt-8 border-t border-white/10 pt-6">
                ...
                <p className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1">
                  <CountUpNumber end={areaStat.value} className="font-[family-name:var(--font-manrope)] text-[2.75rem] font-semibold tabular-nums tracking-tight text-white sm:text-[3rem]" />
                  {areaStat.suffix ? (<span className="font-[family-name:var(--font-manrope)] text-base font-light text-white/55 sm:text-lg">{areaStat.suffix}</span>) : null}
                </p>
                <p className="mt-1.5 font-[family-name:var(--font-manrope)] text-[9.5px] uppercase tracking-[0.28em] text-white/35">As listed for this project</p>
              </div>
            ) : (
              <div className="relative mt-8 border-t border-white/10 pt-6">
                <p className="font-[family-name:var(--font-playfair)] text-[13px] italic text-white/80">Location</p>
                <p className="mt-2 font-[family-name:var(--font-manrope)] text-sm text-white/80">{project.location}</p>
              </div>
            )}
              ...
              <div className="relative mt-5 flex items-center gap-2">
                <div className="flex items-center gap-1">
                  {Array.from({ length: Math.min(slides.length, 8) }).map((_, i) => (
                    <button key={i} type="button" onClick={() => setActiveIndex(i)} className={`h-1 rounded-full transition-all duration-300 ${i === activeIndex ? 'w-5 bg-[#C5A880]' : 'w-1.5 bg-white/20 hover:bg-white/40'}`} aria-label={`View image ${i + 1}`} />
                  ))}
                  {slides.length > 8 ? (<span className="ml-1 font-[family-name:var(--font-manrope)] text-[10px] text-white/30">+{slides.length - 8}</span>) : null}
                </div>
                <span className="font-[family-name:var(--font-manrope)] text-[10px] text-white/30">{slides.length} images</span>
              </div>
            ) : null}

            <div className="relative mt-auto hidden pt-8 font-[family-name:var(--font-manrope)] text-[9.5px] uppercase tracking-[0.25em] text-white/25 lg:block">Scroll for full brief</div>
          </aside>
        </div>
      </section>
    </>
