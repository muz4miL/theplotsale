...
import { lerpBreakpoints, useSectionScrollProgress } from '@/hooks/useSectionScrollProgress';
...
export default function HomeContact() {
  ...
  const p = useSectionScrollProgress(sectionRef);
  const [status, setStatus] = useState('idle');
  const [errorMessage, setErrorMessage] = useState('');

  const imageScale = reduce ? 1 : 1.02 + (1.14 - 1.02) * p;
  const leftTextOpacity = reduce ? 1 : lerpBreakpoints(p, [0, 0.12, 0.42, 0.68, 0.92], [0, 1, 1, 0.75, 0]);
  const leftTextY = reduce ? 0 : lerpBreakpoints(p, [0, 0.18, 0.5, 0.85], [56, 0, 0, -48]);
  const cardOpacity = reduce ? 1 : lerpBreakpoints(p, [0.08, 0.22, 0.55, 0.88], [0, 1, 1, 0.92]);
  const cardY = reduce ? 0 : lerpBreakpoints(p, [0.08, 0.24, 0.55, 0.9], [40, 0, 0, -28]);
    ...
    >
      ...
      <div className="relative grid min-h-[min(100dvh,960px)] lg:grid-cols-2">
        <div className="relative min-h-[48vh] overflow-hidden lg:min-h-[min(100dvh,960px)]">
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-black/75 via-black/25 to-black/55" aria-hidden />
          <div
          ...
          <div
            ...
            style={{ opacity: leftTextOpacity, transform: `translateY(${leftTextY}px)` }}
          >
          ...
          >
            <div className="rounded-2xl border border-white/[0.09] bg-[#0d1815]/95 px-7 py-10 shadow-[0_40px_80px_rgba(0,0,0,0.45)] backdrop-blur-md sm:px-9 sm:py-11 md:rounded-3xl">
              ...
              <div className="mb-6 inline-flex items-center gap-2">
                ...
                <span className="text-[9px] font-medium uppercase tracking-[0.35em] text-[#C5A880]">Concierge</span>
                <Star className="h-3 w-3 text-[#C5A880]" aria-hidden />
              </div>
